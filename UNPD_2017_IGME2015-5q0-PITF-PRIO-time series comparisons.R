...
#setwd("/Users/dakshayanirajabhathor/Desktop/United Nations/IGME-PITF")
...
# unique(IGME.Series$Series.Name)
names(IGME.Series)
...
#If there's no date in Series.Name then create new name with name+date 
...
#xx<-IGME.Series[Series.Type %like% "Vital " & grepl('[0-9]+', IGME.Series$Series.Name)==TRUE,]
#unique(IGME.Series$Series.Type)
#IGME.Series[is.na(IGME.Series$Series.Name2)==FALSE, Series.Name:=Series.Name2]

#If series.name2 column isn't empty then impute value into series.name
...
#xx<- IGME.Series[is.na(Series.Name2)==TRUE & grepl('[0-9]{4}', IGME.Series$Series.Name)==FALSE & is.na(Series.Year)==FALSE]
#IGME.Series[,Series.Name := gsub('-', '', IGME.Series$Series.Name)]
#xx<-as.data.table(unique(IGME.Series$Series.Year))
#xx<-as.data.table(unique(IGME.Series$Series.Name))
#xx<-as.data.table(unique(IGME.Series$Series.Name2))
# str(IGME.Series$Series.Year)
# str(IGME.Series$Series.Name)
# str(IGME.Series$Series.Name2)

#divide data into groups 
#indirect, direct, vr/lt
...
# # check if ISO-countryname match between IGME and external sources (like PITF) 
...
#IGME.all <- merge(IGME.all, prio_acd, by=c("ISOCode","Year"), all.x=TRUE, all.y = TRUE)
#IGME.all <- merge(IGME.all, battledeaths, by=c("Year","ISOCode","IntensityLevel"), all.x=TRUE, all.y = TRUE)
#IGME.PITF<-NULL
#IGME.all<-NULL
...
# # Check if ISO-countryname match between IGME and external source
# test<-PITF_Geno[,1:2]
# test<-unique(test)
# igmetest<-IGMEMR[,1:2]
...
#Remove AVEMAG5 at end of vector when dealing with future copies of PITF_EthnicWar:
...
#no vietnam data
#no korea data in file
#all sudan data look OK except case of "Sudan-North"
...
str(prio_acd)
# unique(prio_acd$IntensityLevel)
# unique(prio_acd$CumulativeIntensity)
# unique(prio_acd$TypeOfConflict) 
...
# write.table(prio_acd, file = "/Users/dakshayanirajabhathor/Desktop/United Nations/IGME-PITF/edited_prio_acd.csv", append = FALSE, quote = TRUE, sep = ",",
...
# write.table(battledeaths, file = "/Users/dakshayanirajabhathor/Desktop/United Nations/IGME-PITF/edited_battledeaths.csv", append = FALSE, quote = TRUE, sep = ",",
#             eol = "\n", na = "NA", dec = ".", row.names = TRUE,
#             col.names = TRUE)
...
# unique(myrainbow3)
# count(myrainbow3)
# #check for max series for a country
# xx<-IGME.Series.all[(is.na(IGME.Series.all$DEATHMAG))==FALSE|(is.na(IGME.Series.all$MAGFATAL))==FALSE|(is.na(IGME.Series.all$IntensityLevel))==FALSE| (is.na(IGME.Series.all$bdeadbest))==FALSE| (is.na(IGME.Series.all$bdeadlow))==FALSE | (is.na(IGME.Series.all$bdeadhigh)==FALSE),]
# xx<-xx[is.na(Series.Name)==FALSE]
# 
# xx<-xx[,c("ISOCode","Series.Name")]
# xx<- unique(xx)
# xa<-count(xx, vars = c("ISOCode"))
# xc<- IGME.Series.all[ISOCode=="INS" & Inclusion=="0"]
# xc<-xc[,c("ISOCode","Series.Name")]
# xd<-unique(xc)
# 
# xx<-NULL
# xsdf<- IGME.Series.all[ISOCode=="INS"]
# xsdf<-xsdf[,c("ISOCode","Series.Name","Series.Type")]
# xsdf<-unique(xsdf)
